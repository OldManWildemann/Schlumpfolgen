print()
print("SchnulpFolgen 4.5C (linux,msieve) by gismo.pracht@proton.me")
...
def factor_with_msieve(number):
    """
    Übergibt eine Zahl an Msieve, liest den Fortschritt live aus
    und gibt eine Liste der gefundenen Primfaktoren zurück.
    """
    # Da Msieve über Homebrew installiert wurde, reicht der Befehl 'msieve'
    # -v schaltet die Detailausgabe ein, damit wir den Fortschritt sehen
    command = ["msieve", "-v", str(number)]
    
    print(f"[*] Msieve {number}")
    
    factors = []
    
    # Startet Msieve als Subprozess
    ...
    # Liest die Ausgabe von Msieve Zeile für Zeile, während das Programm läuft
    for line in process.stdout:
        # Gibt die originale Msieve-Zeile live im Terminal aus
        # sys.stdout.write(line)
        # sys.stdout.flush()
        
        # Regulärer Ausdruck, um die Zeilen mit den Faktoren zu finden.
        # Msieve gibt Faktoren meistens im Format "pX factor: 12345..." aus (z.B. p3 factor, c10 factor)
        if "factor:" in line:
            # Extrahiert die reine Zahl am Ende der Zeile
...
def my_primefactors(number):
    ...
    try:
        ...
        # print("[Timeout Sympy]",base_formatiert,"number",number)
